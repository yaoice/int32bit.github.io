...
tags:
     - openstack
---
本文以制作`Centos7.2`镜像为例，详细介绍制作镜像的步骤，该镜像上传到Openstack Glance中，相对于官方镜像，需要支持如下几个功能：
...
本文宿主机操作系统为`Ubuntu14.04`，开启`VT`功能(使用`kvm-ok`命令验证）并安装了`libvirt`系列工具，包括`virsh`、`virt-manager`、`libguestfs-tools`等。

## 1.下载镜像

直接访问官方[镜像地址](https://www.centos.org/download/mirrors/)下载，注意选择中国的镜像源，相对国外镜像下载速率比较快，进入后选择下载版本为`7.2.1511`，在`isos`目录下下载`x86_64`的`Minimal`镜像，如果网速不给力，最好不要选择下载`Netinstall`镜像，因为这会在安装时联网下载大量的软件包，重新安装时需要重新下载，耗费大量的时间。

## 2.初始化磁盘镜像并启动

创建一个qcow2格式镜像，大小10G足矣。
...
进入虚拟机控制台启动菜单，选择`Install Centos 7`，继续选择语言后将进入`INSTALLION SUMMARY`，其中大多数配置默认即可，`SOFTWARE SELECTION`选择`Minimal Install`，`INSTALLATION DESTINATION`需要选择手动分区，我们只需要一个根分区即可，不需要`swap`分区，文件系统选择`ext4`，存储驱动选择`Virtio Block Device`，如图：

![分区表设置](/Users/fuguangping/github/int32bit.github.io/img/posts/手动制作Openstack镜像/filesystem.png)

配置完成后就可以开始安装了，在`CONFIGURATION`中设置root临时密码，随意设置，自己记住即可，制作完后`cloud-init`将会重新设置root初始密码。

大约几分钟后，即可自动完成安装配置工作，最后点击右下角的reboot重启退出虚拟机。

## 4. 配置OS

安装好系统后，还需要进行配置才能作为glance 镜像使用。首先需要重新启动虚拟机（虽然上一步执行的是reboot，但貌似并不会自动启动)：

```bash
sudo virsh start centos
```

客户的云主机需要支持root ssh远程登录，因此需要开启root远程ssh登录功能，只需要修改配置文件`/etc/ssh/sshd_config`并修改`PermitRootLogin`值为`yes`，重启ssh服务生效:

```bash
sudo systemctl restart sshd
```

接下来的所有操作均通过宿主机ssh登录到虚拟机执行，这样方便复制（默认终端不和宿主机共享粘贴区）。

为了加快安装速度，建议配置本地软件镜像源仓库，若没有本地镜像仓库，则选择国内的软件源，会相对官网的速度下载要快，提高执行效率。

```bash
mv my_repo.repo /etc/yum.repos.d/
```

`hypervisor`需要通过发送对应的信号、事件等到虚拟机中，虚拟机根据接收的信号或者事件执行相应的操作，比如关机、重启等。虚拟机需要开启acpid（Advanced Configuration and Power Interface event daemon）服务，关于acpid的更多资料，参考[Arch文档中关于acpid的介绍以及使用](https://wiki.archlinux.org/index.php/acpid)。为了开启该服务，首先需要安装`acpid`服务，并设置开机自启动：
...
最后安装cloud-init，cloud-init是虚拟机第一次启动时执行的脚本，主要负责完成虚拟机的初始化工作，比如设置主机名、设置初始化密码以及注入密钥等。我们通常会实现自己cloud-init，用来完成虚拟机的一些定制化初始化工作，如果没有自己的cloud-init，直接从官方源下载即可。
...
虚拟机制作镜像时指定了根分区大小（比如我们设置为10GB），为了使虚拟机能够自动调整为flavor disk指定的根磁盘大小，我们需要安装glowpart并完成以下配置：
...
镜像制作完成，上传`centos.qcow2`到`glance`服务中，如果使用ceph作为存储后端，为了提高上传速度（使用glance上传走的http服务，必然很慢），我们需要另辟蹊径，我们只使用glance创建实例记录，并不使用glance上传镜像，包括以下几个步骤：
...
务必设置property的`hw_qemu_guest_agent=yes`,否则libvert启动虚拟机时不会生成qemu-ga配置项，导致虚拟机内部的qemu-ga由于找不到对应的虚拟串行字符设备而失败，提示找不到channel。

## 7.验证

首先使用创建的镜像启动一台新云主机（终于叫云主机，不叫虚拟机了==，如果使用nova CLI，需要传递`--admin-pass`参数并指定密码,并指定`flavor`，要求`disk`大小为20GB。如果使用Openstack dashborad创建，需要简单配置下dashborad使其支持配置虚拟机密码，如图：

![设置密码面板](/Users/fuguangping/github/int32bit.github.io/img/posts/手动制作Openstack镜像/set_password.png)
...
![查看磁盘信息](/Users/fuguangping/github/int32bit.github.io/img/posts/手动制作Openstack镜像/disk.png)
...
首先使用`nova list`获取虚拟机ID，然后调用`nova set-password`命令修改密码:
